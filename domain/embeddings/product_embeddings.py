...
class ProductEmbeddingModel:
    ...
    @staticmethod
    def get_spec_by_key(specifications: List[Dict], key_pattern: str, group: str = None) -> str:
    ...
    def normalize_device_type(categories: List[Dict]) -> str:
        ...
        if any(k in category_text for k in ["phone", "smartphone", "điện thoại", "mobile"]):
            return "smartphone"
    ...
    def create_product_embedding_text(self, product_data: Dict[str, Any]) -> str:
        ...
        cpu = self.get_spec_by_key(specifications, "phiên bản cpu", "Performance") or \
              self.get_spec_by_key(specifications, "cpu", "Performance")
        if cpu:
    ...
    def get_product_recommendation_embedding(self, product_data: Dict[str, Any]) -> np.ndarray:
        """
        ...
        logger.debug(f"Generated recommendation embedding for product {product_data.get('id')}")
    ...
    def create_product_chunks(self, product_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        cpu = self.get_spec_by_key(specifications, "cpu", "Performance") or \
              self.get_spec_by_key(specifications, "phiên bản cpu", "Performance")
        if cpu:
            tech_parts.append(f"CPU: {cpu}")
        
        cpu_cores = self.get_spec_by_key(specifications, "số nhân", "Performance") or \
                    self.get_spec_by_key(specifications, "loại cpu", "Performance")
        if cpu_cores:
        ...
        if screen_size or screen_tech:
            display_text = f"Màn hình: {screen_size or ''} {screen_tech or ''} {screen_std or ''}"
            if resolution:
        ...
        os_name = self.get_spec_by_key(specifications, "tên os", "OperatingSystem")
        os_version = self.get_spec_by_key(specifications, "phiên bản os", "OperatingSystem")
        if os_name:
    ...
    def embed_product_chunks(self, product_data: Dict[str, Any]) -> List[Tuple[np.ndarray, Dict]]:
        """
