...
class SimilaritySearch:
    ...
    def search_by_product_id(
        self, product_id: str, limit: int = 10, threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
        ...
        results = [p for p in similar_products if p["product_id"] != product_id][:limit]
    ...
    def search_by_text(
        ...
        if categories and len(categories) > 0:
            filtered_results = [
                p for p in similar_products if p.get("category") in categories
            ]
