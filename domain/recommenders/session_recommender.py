...
def build_session_transitions(
    ...
    for row in interactions:
        ...
        sessions[session_key].append((ts, pid_s))
    ...
    for session_key, items in sessions.items():
        ...
        for sub in sub_sessions:
            ...
            for i in range(len(sub) - 1):
                ts_from, pid_from = sub[i]
                ts_to, pid_to = sub[i + 1]
                ...
                if pid_to in transitions[pid_from]:
                    ...
                    transitions[pid_from][pid_to] = (old_count + 1, max(old_ts, ts_to))
                else:
                    transitions[pid_from][pid_to] = (1, ts_to)
