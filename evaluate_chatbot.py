...
def evaluate_chatbot():
    ...
    logger.info(f"[INFO] Starting Chatbot Evaluation (Session: {session_id})")
    ...
    for i, case in enumerate(test_cases):
        ...
        try:
            ...
            if case["expected_intent"] and intent.primary != case["expected_intent"]:
                print(
                    f"[WARN] Intent mismatch! Expected: {case['expected_intent']}, Got: {intent.primary}"
                )
            else:
                print(f"[OK] Intent matched.")

            if not answer or "Xin lỗi" in answer[:20]:
                print(f"[WARN] Warning: Bot might have failed to answer.")

            # Print Contexts Brief
            if contexts:
                print(f"📚 Contexts ({len(contexts)}):")
                for ctx in contexts[:3]:
                    c_type = ctx.get("chunk_type") or ctx.get("type", "unknown")
                    c_name = "N/A"
                    metadata = ctx.get("metadata", {}) or ctx.get("data", {})
                    
                    if c_type == "product":
                        c_name = metadata.get("name")
                    elif c_type in ("content", "policy", "cskh"):
                        c_name = metadata.get("title")
                    
                    print(
                        f"   - [{c_type}] {c_name} (Score: {ctx.get('score', 0):.2f})"
                    )
            else:
