...
def apply_temporal_weighting(
    ...
    logger.info(f"Applied temporal weighting (half_life={half_life_days}d) to {len(weighted)} interactions")
    return weighted
...
def add_category_features(
    ...
    if not hasattr(product_store, "get_product"):
        logger.warning("Product store does not support get_product; skipping category features")
        return interactions
