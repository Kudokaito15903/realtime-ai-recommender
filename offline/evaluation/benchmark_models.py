...
class MockUserBehavior:
    ...
    def __init__(self, train_interactions: List[Dict[str, Any]]):
        ...
        sorted_interactions = sorted(
            train_interactions, key=lambda x: x.get("timestamp", 0), reverse=True
        )
    ...
    def get_recent_interactions(
        self, limit: int = 20000, offset: int = 0
    ) -> List[Dict[str, Any]]:
    ...
    def track_view(self, *args):
        pass

    def track_click(self, *args):
        pass
...
def run_benchmark(limit: int = 50000):
    ...
    all_train_items = {
        str(x.get("product_id")) for x in train_data if x.get("product_id")
    }
    ...
    for i, user_id in enumerate(test_users):
        ...
        # 2. ALS
        try:
            als_items = service.get_als_recommendations(
                user_id, limit=20, train_if_missing=False
            )
    ...
    print("\n\n" + "=" * 65)
    print(
        f"{'Model':<20} | {'Recall@20':<10} | {'NDCG@10':<10} | {'HR@10':<10} | {'Coverage':<10}"
    )
    print("-" * 65)
