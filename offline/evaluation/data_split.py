...
def split_data_by_time(
    ...
    try:
        sorted_interactions = sorted(
            interactions, key=lambda x: parse_timestamp(x.get("timestamp"))
        )
    ...
    logger.info(
        f"  Test Set: {len(final_test)} (Dropped {dropped_test_count} cold-start events)"
    )
...
def parse_timestamp(ts: Any) -> float:
    ...
    if isinstance(ts, str):
        ...
        try:
            return time.mktime(
                time.fromisoformat(ts.replace("Z", "+00:00")).timetuple()
            )
        except:
            return 0.0
    return 0.0
