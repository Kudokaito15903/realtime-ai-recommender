...
try:
    ...
    genai = None
    logger.warning(
        "google-generativeai package not found. Chatbot LLM features will be disabled."
    )
...
class ChatbotService:
    ...
    def __init__(
        self, enable_cache: bool = True, redis_host: str = None, redis_port: int = 6379
    ):
    ...
    def _detect_intent_fast(self, query: str) -> Intent:
        ...
        patterns = [
            ...
            (
                r"\b(thanh toán|đổi trả|bảo hành|chính sách|policy|phương thức)\b",
                "policy",
                0.85,
            ),
    ...
    def retrieve(
        self, query: str, top_k: int = 5, use_cache: bool = True, intent: Intent = None
    ) -> List[Dict[str, Any]]:
        # Actual retrieval logic
        embedding = self.embedding_model.get_embedding(query)
        
        # Determine namespace based on intent
        namespace = "rag_chunks"  # Default for products
        if intent and intent.primary in ("policy", "support", "cskh"):
            namespace = "content"
            
        # Vector store search
        results = self.vector_store.find_similar_products(
            embedding=embedding, limit=top_k, namespace=namespace
        )
        # Format results
        return results
    ...
    def retrieve_fast(
        ...
        return self.retrieve(query, top_k=top_k, use_cache=use_cache, intent=intent)
    ...
    def _build_fast_prompt(
        self, query: str, contexts: List[Dict[str, Any]], intent: Intent
    ) -> str:
        ...
        for i, ctx in enumerate(contexts[:3]):  # Limit to top 3
            ...
            # Use data (from DB) or meta (from vector store)
            source_data = data or meta

            if entity_type == "product" and source_data:
                info = self._extract_product_info(data or {}, meta)
...
# ANSWER:"""
    ...
    def answer(
        ...
        try:
            ...
            logger.info(
                f"⚡ Intent: {intent.primary} ({time.time() - start_time:.3f}s)"
            )
            ...
            history_future = self.executor.submit(
                self.get_conversation_history, session_id_safe
            )

            # ✅ Stage 4: Embedding + Retrieval (1-2s)
            resolved_query = query_safe  # Skip coreference for speed
            contexts = self.retrieve_fast(
                resolved_query, top_k=top_k, use_cache=use_cache, intent=intent
            )

            logger.info(
                f"⚡ Retrieved {len(contexts)} contexts ({time.time() - start_time:.3f}s)"
            )
            ...
            # ✅ Stage 4b: Hydrate products with fresh DB data (0.01s - 0.1s)
            # Collect product IDs from contexts
            product_ids = []
            for ctx in contexts:
                meta = ctx.get("metadata", {})
                pid = meta.get("product_id")
                if pid and meta.get("entity_type") != "content":
                    product_ids.append(pid)
            
            # Batch fetch from DB (parallel)
            if product_ids:
                products_map = self.get_products_batch_parallel(product_ids)
                
                # Merge back into contexts
                for ctx in contexts:
                    meta = ctx.get("metadata", {})
                    pid = meta.get("product_id")
                    if pid and pid in products_map:
                        ctx["data"] = products_map[pid]
                        logger.debug(f"Hydrated product {pid} from DB")
            ...
            answer = self._call_genai(
                prompt, max_tokens=max_tokens, temperature=0.1  # Giảm từ 0.2 để faster
            )
            ...
            turn = ConversationTurn(query_safe, answer, contexts, intent, time.time())

            # Async save
            self.executor.submit(self.save_conversation_turn, session_id_safe, turn)
    ...
    def _call_genai_cached(
        ...
        # Cache response
        self.cache.set(cache_key, response, prefix="query_result", ttl=cache_ttl)
...
if __name__ == "__main__":
    ...
    try:
        chatbot = ChatbotService(
            redis_host="localhost", redis_port=6379, enable_cache=True
        )
        ...
        answer, contexts, intent = chatbot.answer(
            "Bên mình bán xe đạp không?", session_id="test"
        )
